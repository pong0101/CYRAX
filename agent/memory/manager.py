...
class MemoryManager:
    ...
    def _result(cls, path: str, score: float | int, content: str) -> dict[str, Any]:
        ...
        return {"path": path, "score": score, "content": content, "evidence": evidence.__dict__}
    ...
    def _ensure_index(self) -> None:
        ...
        for rel, content in self._readable_items(include_logs=False):
            ...
            try:
                ...
                known[rel] = {"hash": digest, "embedding_model": self.embedding_model, "updated": self._now(), "embedding": vector}
                changed = True
            except Exception:
                continue
    ...
    def remember(self, title: str, content: str, folder: str = "01_Memory", memory_type: str = "memory", confidence: str = "HIGH") -> Path:
        ...
        evidence = MemoryEvidence(created=created, updated=now, last_verified=now, memory_type=memory_type, confidence=confidence, status="active")
    ...
    def mark_status(self, path: str, status: str, superseded_by: str = "") -> Path:
        ...
        now = self._now()
        replacements = {"updated": now, "status": status, "superseded_by": superseded_by}
        seen: set[str] = set()
